use generate_parentheses::implementations::{*};
...
fn main() {
    ...
    if args.implementations.is_empty() {
        Arguments::command().error(
            ...
            format!("No implementations requested. Pls choose one from below\n{}", available_implementations_info)
        ).exit();
    }

    for impl_name in args.implementations {
        match find_implementation(impl_name.as_str()) {
            None => Arguments::command().error(ErrorKind::ArgumentNotFound, format!("Unknown implementation: {}. Pls use one below\n{}", impl_name, available_implementations_info)).exit(),
            Some(gen) => {
