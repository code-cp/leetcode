...
class Solution:
    def getMaxGridHappiness(self, m: int, n: int, introvertsCount: int, extrovertsCount: int) -> int:
            ...
            # pre-processing 
            scores = [[0]*max_state for _ in range(max_state)]
            for pre in range(max_state):
                for cur in range(max_state): 
                    scores[pre][cur] = addVal(pre, cur)
            
            people = [None]*max_state
            for state in range(max_state): 
                people[state] = countPeople(state)
                    ...
                    for y in range(extrovertsCount+1): 
                        for state in range(max_state):
                            ...
                            for pre_state in range(max_state):
                                ...
                                dp[i][x][y][state] = max(
                                    dp[i][x][y][state],
                                    dp[i-1][x-a][y-b][pre_state] + scores[pre_state][state]                     
                                    ) 
                            if i == m: 
